...
def tailscale_ip():
    # try the CLI first (standalone install), then scan interfaces for the CGNAT range
...
def lan_ip():
    for iface in ("en0", "en1"):
        try:
            out = subprocess.run(["ipconfig", "getifaddr", iface], capture_output=True, text=True, timeout=4)
            if out.stdout.strip():
...
def cmd_publish(args):
    ...
    if args.open and label != "lan":
        try: subprocess.run(["open", url], timeout=4)
